...
import { IAEntradaPlantio, IASaidaPlantio } from "../schemas/ia-api.schema";
...
export async function getHabilidadesAction(): Promise<ActionResult<string[]>> {
    try {
        ...
        return { error: (error as Error).message || 'Falha ao buscar habilidades.' };
    }
}

export async function searchPlantasAction(search?: string, pagina?: number): Promise<ActionResult<ListagemPlantas>> {
    try {
        console.log('[DEBUG] searchPlantasAction:', search, pagina);
        const response = await Repositories.plantas.searchPlantas(search, pagina);
        return { data: response.data }; 
    } catch (error) {
        console.error('Error in searchPlantasAction:', error);
        return { error: (error as Error).message || 'Falha ao buscar plantas.' };
    }
}

export async function getPlantaAction(id: number): Promise<ActionResult<Planta>> {
    try {
        ...
        return { error: (error as Error).message || 'Falha ao buscar detalhes da planta.' };
    }
}

export async function processarPlantioIAAction(payload: IAEntradaPlantio): Promise<ActionResult<IASaidaPlantio>> {
    try {
        ...
        return { error: (error as Error).message || 'Falha ao processar o plantio.' };
    }
} 