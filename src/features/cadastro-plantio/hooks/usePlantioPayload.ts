...
export type CondicaoAmbienteFormType = z.infer<typeof AmbientesSchema.shape.condicao>;
export type SistemaCultivoType = z.infer<typeof SistemaCultivoEnum>;
export type CondicaoAmbienteIAType = z.infer<typeof IAAmbienteSchema.shape.condicao>;
...
function mapCondicaoAmbienteToIA(condicao: CondicaoAmbienteFormType): CondicaoAmbienteIAType {
  switch (condicao) {
    case 'interno':
      return 'interno';
    case 'externo':
    ...
    case 'comunitario':
      return 'externo';
    default:
      console.warn(`Condição de ambiente não mapeada: ${condicao}, usando 'externo'.`);
      return 'externo';
  }
}
...
export function usePlantioPayload({
  ...
  const [plantioPayload, setPlantioPayload] = useState<IAEntradaPlantio | null>(null);
  const [payloadJson, setPayloadJson] = useState<string | null>(null);
  const [payloadValidationError, setPayloadValidationError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    startProcessingTransition(() => {
      ...
      const mappedCondicaoAmbiente = mapCondicaoAmbienteToIA(condicaoAmbiente);

      const builtPayload: Omit<IAEntradaPlantio, "planta"> & { planta: Partial<IAEntradaPlantio["planta"]> } = {
        data_inicio_plantio: dataInicioPlantio,
        planta: {
          nome: plantaDetalhes.nome,
          nome_cientifico: plantaDetalhes.nomeCientifico || plantaDetalhes.nome,
        ...
        habilidades_existentes: habilidadesUsuario && habilidadesUsuario.length > 0 ? habilidadesUsuario : [],
      };
      
      const finalPlantaPayload = Object.fromEntries(
        Object.entries(builtPayload.planta).filter(([, value]) => value !== undefined)
      ...
      const parsedResult = IAEntradaPlantioSchema.safeParse(finalBuiltPayload);
      ...
      if (parsedResult.success) {
        ...
        console.error("Erro ao validar payload para API de IA:", parsedResult.error.flatten());
        setPlantioPayload(null);
        setPayloadJson(null);
        setPayloadValidationError("Erro na validação dos dados para IA: " + parsedResult.error.message);
      }
    });
  }, [
    quantidade,
